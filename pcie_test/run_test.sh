...
##############################################################################
# PCIe Bandwidth Saturation Test - Background Runner
# 
# Assumes MIG partitions are already created.
# Tests PCIe bandwidth fairness and contention across all MIG instances.
# Simultaneous bidirectional transfers (H2D and D2H) on all devices.
##############################################################################

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LOG_DIR="${SCRIPT_DIR}/logs"
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
PID_FILE="${LOG_DIR}/pcie_test_${TIMESTAMP}.pid"
...
echo "======================================"
echo "PCIe Bandwidth Saturation Test"
echo "======================================"
echo ""
...
# Run the PCIe test in background
echo "Starting PCIe bandwidth test in background..."
echo ""

nohup bash "${SCRIPT_DIR}/pcie_stress_test.sh" > "$BACKGROUND_LOG" 2>&1 &
STRESS_PID=$!

# Save PID
echo "$STRESS_PID" > "$PID_FILE"

echo "PCIe bandwidth test started successfully!"
echo ""
...
echo "  tail -f ${LOG_DIR}/pcie_test_*.log"
echo ""
